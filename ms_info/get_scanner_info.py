...
def get_scanner_info(c, out):
    ...
    for idx in range(len(df)):
        ...
        dicom = glob("/working/henry_temp/PBR/dicoms/{0}/E*/*/*.DCM".format(mse))
        if len(dicom) > 0:
            dicom = dicom[0]
            cmd = ["dcmdump", dicom]
            try:
        ...
        """dicom_txt = glob("/working/henry_temp/PBR/dicoms/{0}/E*/*/*.txt".format(mse))
        if len(dicom_txt) >0:
            try:
                dicom_txt = dicom_txt[0]
                txt = open(dicom_txt, "r")
                txt_line = txt.readlines()
                for line in txt_line:
                    if "SoftwareVersions" in line:
                        print(line)
                        df.loc[idx,"SoftwareVersions"] = line 
            except:
                pass"""


        """if os.path.exists(_get_output(mse) + '/' +mse+ "/alignment/baseline_mni/"):
            df.loc[idx, 'BL MNI Registered'] = "yes"
        else:
            df.loc[idx, 'BL MNI Registered'] = "no"

        if os.path.exists(_get_output(mse) + '/' + mse + '/sienaxorig_flair/'):
            df.loc[idx, 'sienax_directory'] = "sienaxorig_flair"
        elif os.path.exists(_get_output(mse) + '/' + mse + '/sienaxorig_t2/'):
            df.loc[idx, 'sienax_directory'] = "sienaxorig_t2"
        elif os.path.exists(_get_output(mse) + '/' + mse + '/sienax_flair/'):
            df.loc[idx, 'sienax_directory'] = "sienax_flair"
        elif os.path.exists(_get_output(mse) + '/' + mse + '/sienax_t2/'):
            df.loc[idx, 'sienax_directory'] = "sienax_t2"
        else:
            df.loc[idx, 'sienax_directory'] = "sienx has not been run"""
